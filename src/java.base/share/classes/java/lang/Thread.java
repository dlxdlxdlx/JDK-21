 ...
 * {@link #run() run}
 * method. The newly started thread executes concurrently with the thread that
 * caused
 * it to start.
 *
 * <p>
 * A thread <i>terminates</i> if either its {@code run} method completes
 * normally,
 ...
 * #getName() name}. The identifier is generated when a {@code Thread} is
 * created
 * and cannot be changed. The thread name can be specified when creating a
 * thread
 ...
 * local to a thread, meaning a thread can have a copy of a variable that is set
 * to
 * a value that is independent of the value set by other threads. {@code Thread}
 * also
 * supports {@link InheritableThreadLocal} variables that are thread local
 * variables
 ...
 * <p>
 * {@code Thread} supports the creation of <i>platform threads</i> that are
 ...
 * maintained by the operating system. Platforms threads are suitable for
 * executing
 ...
 * thread (the thread that typically calls the application's {@code main}
 * method).
 * The <a href="Runtime.html#shutdown">shutdown sequence</a> begins when all
 * started
 * non-daemon threads have terminated. Unstarted non-daemon threads do not
 * prevent
 ...
 * <p>
 * {@code Thread} also supports the creation of <i>virtual threads</i>.
 * Virtual threads are typically <i>user-mode threads</i> scheduled by the Java
 * runtime rather than the operating system. Virtual threads will typically
 * require
 * few resources and a single Java virtual machine may support millions of
 * virtual
 * threads. Virtual threads are suitable for executing tasks that spend most of
 * the time blocked, often waiting for I/O operations to complete. Virtual
 * threads
 ...
 * <em>carrier threads</em>. Locking and I/O operations are examples of
 * operations
 * where a carrier thread may be re-scheduled from one virtual thread to
 * another.
 ...
 * the {@link #start() start} method to schedule threads to execute.
 * {@code Thread}
 ...
 * platform and virtual threads. The following are examples that use the
 * builder:
 * {@snippet :
 * Runnable runnable = ...
 *
 ...
 * ...
 * ThreadFactory factory = Thread.ofPlatform().daemon().name("worker-",
 * 0).factory();
 *
 ...
 * A {@code Thread} inherits its initial values of
 * {@linkplain InheritableThreadLocal
 * inheritable-thread-local} variables (including the context class loader) from
 * the parent thread values at the time that the child {@code Thread} is
 * created.
 * The 5-param
 * {@linkplain Thread#Thread(ThreadGroup, Runnable, String, long, boolean)
 ...
 * {@link Builder#inheritInheritableThreadLocals(boolean)
 ...
 *
 * <p>
 * Creating a platform thread {@linkplain AccessController#getContext()
 * captures} the
 * {@linkplain AccessControlContext caller context} to limit the
 * {@linkplain Permission
 * permissions} of the new thread when it executes code that performs a
 * {@linkplain
 * AccessController#doPrivileged(PrivilegedAction) privileged action}. The
 * captured
 ...
 *
 * @implNote
 *           In the JDK Reference Implementation, the virtual thread scheduler
 *           may be configured
 ...
 *           </th>
 *           <td>The maximum number of platform threads available to the
 *           scheduler.
...
public class Thread implements Runnable {
     ...
     * non-zero when the thread is started, and reset to zero when the thread
     * terminates.
    ...
    /*
     * 与本线程相关的 ThreadLocal 值。此映射由 ThreadLocal 类维护。
     * 
    ...
    /*
     * 与本线程相关的可继承 ThreadLocal 值。此映射由 InheritableThreadLocal 类维护。
     * 
     ...
     *                              sleeping. The
     *                              <i>interrupted status</i> of the current thread
     *                              is
     *                              cleared when this exception is thrown.
     *
     * @since 19
    ...
    /**
     * Returns a builder for creating a platform {@code Thread} or
     * {@code ThreadFactory}
     * that creates platform threads.
     *
     * <p>
     * <a id="ofplatform-security"><b>Interaction with security manager when
     ...
     *          // must be invoked to schedule it to execute.
     *          Thread thread =
     *          Thread.ofPlatform().name("duke").unstarted(runnable);
     *
     *          // A ThreadFactory that creates daemon threads named "worker-0",
     *          "worker-1", ...
     *          ThreadFactory factory = Thread.ofPlatform().daemon().name("worker-",
     *          0).factory();
     *          }
     *
     * @return A builder for creating {@code Thread} or {@code ThreadFactory}
     *         objects.
     * @since 21
     */
    ...
    /**
     * Returns a builder for creating a virtual {@code Thread} or
     * {@code ThreadFactory}
     ...
     * {@code Thread} or {@code ThreadFactory} is created with the following
     * methods:
     *
     * <ul>
     ...
     * <li>The {@linkplain #factory() factory} method creates a
     * {@code ThreadFactory}.
     * </ul>
     *
     * <p>
    ...
    public sealed interface Builder
         ...
         *
         * @apiNote
         *          The following example creates a builder that is invoked twice to
         *          start
         ...
         *                           (See <a href=
         *                           "Thread.html#ofplatform-security">Interaction with
        ...
        sealed interface OfPlatform extends Builder
             ...
             * 
             * @implSpec The default implementation invokes {@linkplain #daemon(boolean)}
             *           with
     ...
     *                           if the current thread cannot create a thread in the
     *                           specified
     *                           thread group or cannot override the context class
     *                           loader methods.
     *
     * @see <a href="#inheritance">Inheritance when creating threads</a>
     */
     ...
     *
     * @param group
     *                                       the thread group. If {@code null} and
     *                                       there is a security
     ...
     *                                       {@code
     *         SecurityManager.getThreadGroup()} returns {@code null}, the group
     *                                       is set to the current thread's thread
     *                                       group.
     *
     * @param task
     *                                       the object whose {@code run} method is
     *                                       invoked when this thread
     *                                       is started. If {@code null}, this
     ...
     *                                       thread, or zero to indicate
     *                                       that this parameter is to be ignored
     *
     * @param inheritInheritableThreadLocals
     *                                       if {@code true}, inherit initial values
     *                                       for inheritable
     *                                       thread-locals from the constructing
     *                                       thread, otherwise no initial
     ...
     * platform thread created with a {@link Runnable} task then invoking this
     * method
     ...
     *           the {@code Thread} was created with. If the thread was created
     *           without a task
     ...
     *             of the {@code ThreadDeath} exception propagating up the stack).
     *             If
     ...
     *             <a href=
     *             "{@docRoot}/java.base/java/lang/doc-files/threadPrimitiveDeprecation.html">Why
     ...
     *           that is not alive still records that the interrupt request was made
     *           and
     *           will report it via {@link #interrupted()} and
     *           {@link #isInterrupted()}.
     *
     * @throws SecurityException
     ...
     *             It was inherently deadlock-prone. If the target thread held a
     *             lock on
     *             a monitor protecting a critical system resource when it was
     *             suspended,
     *             no thread could access the resource until the target thread was
     *             resumed.
     *             If the thread intending to resume the target thread attempted to
     *             lock
     *             the monitor prior to calling {@code resume}, deadlock would
     *             result.
     *             Such deadlocks typically manifested themselves as "frozen"
     *             processes.
     *             For more information, see
     *             <a href=
     *             "{@docRoot}/java.base/java/lang/doc-files/threadPrimitiveDeprecation.html">Why
     ...
     *                                  {@code MAX_PRIORITY}.
     * @throws SecurityException
     *                                  if {@link #checkAccess} determines that the
     *                                  current
     ...
     *           current thread, and it's a platform thread that was not attached to
     *           the
     *           VM with the Java Native Interface
     *           <a href=
     *           "{@docRoot}/../specs/jni/invocation.html#attachcurrentthread">
     *           AttachCurrentThread</a> function, then this method will set the
     *           operating
     *           system thread name. This may be useful for debugging and
     *           troubleshooting
     *           purposes.
     *
     ...
     * {@link java.lang.ThreadGroup#enumerate(Thread[])}
     ...
     *           {@code this.wait}
     *           calls conditioned on {@code this.isAlive}. As a thread terminates
     *           the
     ...
     *                                  if the value of {@code millis} is negative,
     *                                  or the value
     *                                  of {@code nanos} is not in the range
     *                                  {@code 0-999999}
     *
     * @throws InterruptedException
     *                                  if any thread has interrupted the current
     ...
     *                                     {@code on} is false
     * @throws IllegalThreadStateException
     *                                     if this thread is {@linkplain #isAlive
     *                                     alive}
     * @throws SecurityException
     *                                     if {@link #checkAccess} determines that
     *                                     the current
     *                                     thread cannot modify this thread
     */
     ...
     *
     * <p>
     * The context {@code ClassLoader} of the primordial thread is typically
     ...
     *                           class loader
     *                           is not {@code null} and is not the same as or an
     *                           ancestor of the
     *                           context class loader, and the caller does not have
     *                           the
    ...
    private static class Caches {
        /** cache of subclass security audit results */
        static final ClassValue<Boolean> subclassAudits = new ClassValue<>() {
            @Override
